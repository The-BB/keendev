...
<div class="col2">
	...
	if (!empty($settings['show']['os']))
		$core[] = array($lang['os'], ($show_icons && (file_exists(LOCAL_PATH . 'layout/icons/os_'.$os_icon.'.gif') || file_exists(LOCAL_PATH . 'layout/icons/os_'.$os_icon.'.png')) ? '<span class="icon icon_os_'.$os_icon.'"></span>' : '') . $info['OS']=shell_exec("uname -sm && ndmq -p \"show version\" -P release"));
	...
	if (!empty($settings['show']['kernel']))
		$core[] = array($lang['kernel'], $info['Kernel']=shell_exec("cat /proc/version"));
		
	// Model?
	if (!empty($settings['show']['model']) && array_key_exists('Model', $info) && !empty($info['Model']))
		$core[] = array($lang['model'], $info['Model']=shell_exec("ndmq -p \"show version\" -P device"));
	...
	if (!empty($settings['show']['cpu'])) {
		$cpus = shell_exec("grep -o 'Ralink.*' /proc/cpuinfo"); print "$cpu[$cpu]";
	...
	if (!empty($settings['show']['cpu']) && array_key_exists('CPUArchitecture', $info)) 
		$core[] = array($lang['cpu_arch'], $info['CPUArchitecture']=shell_exec("grep -o 'model.*' /proc/cpuinfo | grep -o 'M.*' | uniq"));
